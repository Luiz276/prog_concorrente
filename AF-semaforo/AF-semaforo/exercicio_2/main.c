...
void ponte_inicializar() {
	...
	sem_init(&ilha, 0, 0);
	cabeceira_t cont;
	sem_init(&cont, 0, veiculos_turno);
	pthread_mutex_init(&sair, NULL);
	pthread_mutex_lock(&sair);
...
void ponte_entrar(veiculo_t *v) {
	...
	if (v->cabeceira == CONTINENTE) {
		printf("\n[Veiculo   %d] Aguardando para entrar na ponte pelo(a) %s", v->id, v->cabeceira);
		fflush(stdout);
		sem_wait(&cont);
	} else {
		printf("\n[Veiculo   %d] Aguardando para entrar na ponte pelo(a) %s", v->id, v->cabeceira);
...
void ponte_sair(veiculo_t *v) {
	...
	int aux;
	if (v->cabeceira == CONTINENTE) {
		sem_getvalue(&cont, aux);
		while (aux > 0);
		sem_post(&ilha);
	} else {
		sem_getvalue(&ilha, aux);
		while (aux > 0);
		sem_post(&cont);
	}
